...
describe('templates (real registry)', () => {
  it('has exactly 14 templates covering both orientations', () => {
    expect(templates).toHaveLength(14);
    expect(templates.filter(t => t.orientation === 'vertical')).toHaveLength(7);
    expect(templates.filter(t => t.orientation === 'horizontal')).toHaveLength(7);
